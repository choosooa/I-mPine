...
- 입력: CH2_전체병합패널_5도메인_2016_2023_최종보정4.csv
...
CORE = {
    ...
    "인위적확산": ["log1p_원목생산업체수"],  # 도로소나무비율_500m 제외(면적통제 후 상관 0.0008로 붕괴, 최종보정4 재검증)
}
AREA = "log_소나무림면적"
...
# 저장
# =========================================================
contrib.to_csv(f"{MODELDIR}/Model1_기여도분해_유형화_final.csv", index=False, encoding="utf-8-sig")
main_out_cols = ["연도", "시도", "시군구", Y, "Y_bin"] + dom_cols + ["구조적취약성지수", "위험지수"]
model_df[model_df["연도"].between(2016, 2023)][main_out_cols].to_csv(
    f"{MODELDIR}/구조적취약성지수_final.csv", index=False, encoding="utf-8-sig"
)
with open(f"{MODELDIR}/Model1_final_로그.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(LOG))
log(f"\n저장 완료: Model1_기여도분해_유형화_final.csv, 구조적취약성지수_final.csv, Model1_final_로그.txt")
