# -*- coding: utf-8 -*-
"""
Model 1 (구조적 취약성지수) v4 - TierA 도로소나무비율 반영판
- 원본 model1_v4_final.py를 기반으로 PANEL을 최종보정4.csv(TierA)로 교체, 경로를 로컬로 조정.
- 변경점(v3 대비): 평균풍속_ms를 기후위험도 도메인에서 제외(관측소 공유지역 교란효과 확인).
  기후위험도 도메인 = 연강수량_mm, GDD_솔수염하늘소_base11.9 (2개, SPI3는 v3에서 이미 제외)
- 나머지 설계(동일가중, CRE 2부문모형, 배분갭, 기여도분해, 2축 유형화)는 v3와 동일.
- 도로소나무비율_500m은 아직 그대로 포함(제외 여부는 이 결과 보고 판단).
"""
import numpy as np
import pandas as pd
...
MODELDIR = r"C:\Users\SAMSUNG\OneDrive\바탕 화면\공모전\Final_file\I-m_Pine\Modeling\2차"
PANEL = MODELDIR + r"\CH2_전체병합패널_5도메인_2016_2023_최종보정4.csv"
...
log(f"\n[면적통제 후 재해석, TierA] 도로소나무비율_resid vs Y: r={r_road:+.3f} (사전 명시: +)")
log(f"[면적통제 후 재해석, TierA] 원목업체_resid vs Y: r={r_wood:+.3f} (사전 명시: -)")
...
log(f"\n{'='*70}\n[훈련 2017-2020] 구조적취약성지수(v4, TierA)\n{'='*70}")
run_two_part(train, "구조적취약성지수", "훈련")
log(f"\n{'='*70}\n[홀드아웃 2021-2022] 구조적취약성지수(v4, TierA)\n{'='*70}")
...
log(f"\n{'='*70}\n취약성지수 상위 20개 시군구 - 도메인별 기여도 (v4, TierA)\n{'='*70}")
...
# 저장
# =========================================================
contrib.to_csv(f"{MODELDIR}/Model1_기여도분해_유형화_v4_TierA.csv", index=False, encoding="utf-8-sig")
main_out_cols = ["연도", "시도", "시군구", Y, "Y_bin"] + dom_cols + ["구조적취약성지수", "위험지수"]
model_df[model_df["연도"].between(2016, 2023)][main_out_cols].to_csv(
    f"{MODELDIR}/구조적취약성지수_v4_TierA.csv", index=False, encoding="utf-8-sig"
)
with open(f"{MODELDIR}/Model1_v4_TierA_로그.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(LOG))
log(f"\n저장 완료: Model1_기여도분해_유형화_v4_TierA.csv, 구조적취약성지수_v4_TierA.csv, Model1_v4_TierA_로그.txt")
